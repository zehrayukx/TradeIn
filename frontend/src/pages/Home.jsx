...
const Home = ({ isLoggedIn, setIsLoggedIn }) => {
  ...
  // 🎯 YENİ: Sidebar'daki bir Trende (Hashtag) tıklandığında çalışır
  const handleTrendSidebarClick = (hashtag) => {
    setSearchQuery(hashtag); // 1. Arama barını doldurur
    window.scrollTo({ top: 0, behavior: 'smooth' }); // Ekranı en üste kaydırır
    
    // 2. Üstteki sekmelerden biriyle eşleşiyorsa o sekmeyi yakar, eşleşmiyorsa 'Trendler'de bırakır
  ...
  const handleLike = async (postId) => {
    ...
    try {
      ...
      setPosts(prevPosts => prevPosts.map(post => post.id === postId ? { ...post, likes: response.data.begenildi ? post.likes + 1 : Math.max(0, post.likes - 1), isLiked: response.data.begenildi } : post));
  ...
  // 🚀 KİLİT BÖLÜM: searchQuery veya activeTab değiştiğinde burası otomatik tetiklenir!
  useEffect(() => {
    const fetchPosts = async () => {
      ...
      try {
        ...
        // Eğer arama kutusunda bir şey varsa DAİMA o hashtag'i backend'e yolla
        if (searchQuery) {
    ...
    fetchPosts();
  }, [activeTab, isLoggedIn, searchQuery]); // 🎯 DİKKAT: searchQuery buraya eklendi!
  ...
  return (
    <div className="min-h-screen bg-[#0a0f1d] text-slate-100 flex flex-col relative">
      ...
      <Navbar 
        ...
        searchQuery={searchQuery} // 🎯 YENİ: State'i Navbar'a pasla
      ...
      {tab}
          ...
          <div className="flex flex-col gap-4">
            {loading ? (
              ...
              <>
                {/* 🎯 Eğer arama aktifse ufak bir bilgi banner'ı gösterelim */}
                {searchQuery && (
        ...
        <aside className="w-[300px] hidden lg:block py-6 pr-6 shrink-0">
          {/* 🎯 YENİ: Tıklama fonksiyonunu Sidebar'a paslıyoruz! */}
          <TrendSidebar onTrendClick={handleTrendSidebarClick} />
          <RecommendedTraders />
        </aside>
      </div>

      {/* CREATE POST MODAL AYNI */}
      {showCreateModal && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center z-[100] p-4 transition-all">
          <div className="bg-[#1a1d26] border border-gray-800 rounded-3xl max-w-xl w-full shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="px-6 py-4 border-b border-gray-800 flex items-center justify-between">
              ...
              <button onClick={() => setShowCreateModal(false)} className="p-2 hover:bg-gray-800 rounded-full transition-colors text-gray-400 hover:text-white"><X size={20} /></button>
            </div>
            <div className="p-6">
              <div className="flex gap-4">
                ...
                <div className="flex-1">
                  <textarea autoFocus value={newPostContent} onChange={(e) => setNewPostContent(e.target.value)} placeholder="Piyasalarda neler oluyor?" className="w-full bg-transparent border-none text-lg text-white placeholder-gray-600 resize-none outline-none min-h-[150px] py-2" />
                  <div className="flex items-center justify-between pt-4 border-t border-gray-800/50 mt-4">
                    ...
                    <button disabled={!newPostContent.trim() || isPublishing} onClick={handlePublishPost} className="bg-blue-600 hover:bg-blue-500 disabled:bg-gray-700 disabled:text-gray-500 text-white px-6 py-2 rounded-xl font-bold transition-all flex items-center gap-2 shadow-lg shadow-blue-600/20">{isPublishing ? 'Yayınlanıyor...' : <><Send size={18} /> Paylaş</>}</button>
                  </div>
                </div>
              </div>
            </div>
