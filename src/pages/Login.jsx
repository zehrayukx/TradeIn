...
const Login = () => {
  ...
  return (
    <div className="min-h-screen bg-[#0a0f1d] text-white flex flex-col">
        ...
        <div className="w-full max-w-5xl grid md:grid-cols-2 gap-12 items-center">
          ...
          <div className="hidden md:block space-y-8">
            ...
            <div className="relative h-32 w-full opacity-40">
              {/* Buraya görseldeki mavi grafik dalgası gelecek (SVG veya PNG) */}
          ...
          <div className="bg-[#161b22] border border-white/5 p-10 rounded-[32px] shadow-2xl">
            ...
            <p className="text-slate-400 text-sm mb-8">Hesabına giriş yaparak devam et</p>

            <form onSubmit={handleSubmit} className="space-y-5">
              <InputGroup label="E-posta veya Kullanıcı Adı" icon={<Mail size={18}/>} placeholder="ornek@email.com veya @kullaniciadi" 
                value={formData.identifier} onChange={(e) => setFormData({...formData, identifier: e.target.value})} />
              
              <div className="relative">
                <InputGroup label="Şifre" icon={<Lock size={18}/>} type={showPassword ? "text" : "password"} placeholder="Şifrenizi girin"
      ...
      <footer className="p-6 text-center text-[10px] text-slate-600">
        Devam ederek, <span className="text-blue-500 cursor-pointer">Kullanım Şartları</span> ve <span className="text-blue-500 cursor-pointer">Gizlilik Politikası</span>'nı kabul etmiş olursunuz.
      </footer>
    </div>
  );
};
...
const SocialButton = ({ icon, text }) => (
  <button className="w-full flex items-center justify-center gap-3 py-3 border border-white/5 rounded-xl text-sm font-bold hover:bg-white/5 transition-all">
