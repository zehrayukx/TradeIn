...
def send_verification_email(to_email: str, code: str):
    ...
    subject = "TradeIn - E-posta Doğrulama Kodunuz"

    # Senin mevcut şifre sıfırlama tasarımıyla uyumlu premium hoş geldin şablonu
    html_content = f"""
    <html>
        <body style="font-family: Arial, sans-serif; background-color: #f8fafc; padding: 20px;">
            <div style="max-width: 500px; margin: 0 auto; background-color: #ffffff; padding: 24px; border-radius: 16px; border: 1px solid #e2e8f0; text-align: center;">
                ...
                <hr style="border: 0; border-top: 1px solid #f1f5f9; margin-bottom: 20px;">
                <p style="color: #334155; font-size: 16px; text-align: left;">Merhaba,</p>
                <p style="color: #334155; font-size: 15px; text-align: left;">TradeIn platformuna hoş geldiniz! Üyeliğinizi tamamlamak ve hesabınızı aktifleştirmek için 6 haneli doğrulama kodunuz aşağıdadır:</p>
                
                <div style="background-color: #eff6ff; border: 1px dashed #3b82f6; padding: 15px; border-radius: 8px; margin: 25px 0; font-size: 28px; font-weight: 900; letter-spacing: 8px; color: #1d4ed8;">
                    {code}
                </div>
                
                <p style="color: #64748b; font-size: 13px; text-align: left;">Bu kod <b>10 dakika</b> boyunca geçerlidir. Güvenliğiniz için kodu kimseyle paylaşmayınız.</p>
            </div>
        </body>
    </html>
    """
    ...
    try:
        with smtplib.SMTP("smtp.gmail.com", 587) as server:
            ...
            server.login(SENDER_EMAIL, APP_PASSWORD)
            server.sendmail(SENDER_EMAIL, to_email, msg.as_string())
        print(f"✅ [SMTP DOĞRULAMA]: Üyelik kodu {to_email} adresine gönderildi!")
    except Exception as e: 
        print(f"❌ [SMTP DOĞRULAMA HATA]: {e}")